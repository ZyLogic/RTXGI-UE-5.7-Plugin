	...
	FGlobalIlluminationPluginResources& Resources)
{
	...
	const uint32 DEBUG_FRAME_INTERVAL = 120; // Every 2 seconds at 60fps
    
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	// Only print debug when we have performance stats enabled
	if (FrameCounter % DEBUG_FRAME_INTERVAL == 0)
	{
		DebugShaderPlatformsDetailed();
        
		// You can also add conditional logging based on performance
		float rtxgiUpdateTime = UpdateTimer.GetTimeMS();
		if (rtxgiUpdateTime > 16.67f) // Longer than 60fps frame budget
		{
			UE_LOG(LogTemp, Warning, TEXT("DDGI Performance Warning - Update Time: %.2fms"), rtxgiUpdateTime);
		}
	}
#endif
    
	FrameCounter++;*/
	// Safe shader map access with logging
